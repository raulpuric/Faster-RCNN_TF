...
class rgbd(imdb):
    def __init__(self, image_set, year, devkit_path=None):
        imdb.__init__(self, 'rgb_d_' + year + '_' + image_set)
        self._year = year
    ...
    def _get_default_path(self):
        ...
        """
        return os.path.join('/home/raulpuric/ken/household_data', 'rgbd-' + self._year)
...
#            objs = non_diff_objs
        ...
        for ix, obj in enumerate(objs):
            ...
            x2 = max(float(bbox.find('xmax').text) - 1,0.0)
            y2 = max(float(bbox.find('ymax').text) - 1,0.0)
